...
public final class RepoAccess {
    ...
    public static boolean isAdmin(Connection db, int userId) throws Exception {
    ...
    public static boolean canWrite(Connection db, int userId, int repoId) throws Exception {
        if (isAdmin(db, userId))
            return true;
    ...
    public static boolean canAdmin(Connection db, int userId, int repoId) throws Exception {
        if (isAdmin(db, userId))
            return true;
    ...
    public static void requireRead(Connection db, int userId, int repoId) throws Exception {
        if (!canRead(db, userId, repoId))
            throw new UserException("You do not have access to this repository.");
    }

    public static void requireWrite(Connection db, int userId, int repoId) throws Exception {
        if (!canWrite(db, userId, repoId))
            throw new UserException("You do not have write access to this repository.");
    }

    public static void requireAdmin(Connection db, int userId, int repoId) throws Exception {
